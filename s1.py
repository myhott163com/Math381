...
def pairs(alphas=[10, 15, 20], gammas=[0, 1, 2, 3]):
    """
    parameters:
        alphas: All alpha values we will try.
        gammas: All gamma values we will try.
    Return all pairs [alpha, gamma] we will try.
    """
    # get all combinations of alphas and gammas
    combinations = []
    for alpha in alphas:
        for gamma in gammas:
            combinations.append([alpha, gamma])

    # get all pairs [alpha, gamma] we will try
...
# prepare all [alpha, gamma] pairs we will try
pairs_try = pairs()
    ...
    for game in range(numberOfGames): ## main game loop
        ...
        while(cardIndex<24):
            ...
            if (currentPlayer in [0]):
                ...
                # pick up the card if (1) it is adjacent to a card the player already has, and there are at least gamma tokens on it or
            ...
            if (currentPlayer in [1,2,3]):
                ...
                # (3) the player has no tokens
                if ( (((currentCard+1 in players[currentPlayer].cards) or (currentCard-1 in players[currentPlayer].cards))
                and (currentTokens >= player_others[1]))
                or (currentCard-currentTokens<cardThresh) or (players[currentPlayer].tokens==0)):
                ## currentPlayer takes the card and the tokens, if any
                    players[currentPlayer].tokens = players[currentPlayer].tokens+currentTokens
                    players[currentPlayer].cards.append(currentCard)
